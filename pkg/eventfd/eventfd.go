...
func (ev Eventfd) Notify() error {
	return ev.write([sizeofUint64]byte{1, 0, 0, 0, 0, 0, 0, 0})
}
...
func (ev Eventfd) write(val [sizeofUint64]byte) error {
	for {
		n, err := unix.Write(ev.fd, val[:])
		if err == unix.EINTR {
			continue
		}
		if n != sizeofUint64 {
...
// Wait blocks untils eventfd is non-zero (i.e. someone calls Notify or Write).
